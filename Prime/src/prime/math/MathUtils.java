...
public final class MathUtils {
	...
	return new Vec3f(sincos * xx + sinsin * yx + costh * zx, sincos * xy
		+ sinsin * yy + costh * zy, sincos * xz + sinsin * yz + costh
		* zz);
    }
     ...
     * @param dest
     */
    public static void stratifiedScatteredDirection(Vec3f srcDir, int n, int i,
	    int j, int nSamples, Vec3f dest) {
	float zx = srcDir.x, zy = srcDir.y, zz = srcDir.z;
	float xx = zy, xy = -zx, xz = 0;
	float invLen = 1.0f / (float) Math.sqrt(xx * xx + xy * xy + xz * xz);
	xx *= invLen;
     ...
     * @param dest
     */
    public static void stratiefiedRandomReflectDirection(Vec3f srcDir, int i,
	    int j, int n, Vec3f dest) {
	float zx = srcDir.x, zy = srcDir.y, zz = srcDir.z;
	float xx = zy, xy = -zx, xz = 0;
	float invLen = 1.0f / (float) Math.sqrt(xx * xx + xy * xy + xz * xz);
	xx *= invLen;
     ...
     * @param dest
     */
    public static void stratifiedRandomDirectionInHemisphere(Vec3f normal,
	...
	Vec3f ret = new Vec3f((float) (1 - 2 * random()),
		(float) (1 - 2 * random()), (float) (1 - 2 * random()));
	ret.normalize();
	return ret;
    }
